...
const { buildPaymentUrl, verifyReturnUrl } = require('../utils/vnpayHelpers');
...
class ControllerPayments {
    ...
    getClientIp = (req) => {
        const forwarded = String(req.headers['x-forwarded-for'] || '')
            .split(',')[0]
            .trim();
        const raw = forwarded || req.socket?.remoteAddress || req.ip || '127.0.0.1';
    ...
    paymentVnpay = async (req, res) => {
        try {
            ...
            const amounts = this.calculateCartAmounts(cart);
            const amount = Math.round(Number(amounts.total) || 0);

            if (!Number.isFinite(amount) || amount < 1000 || amount > 1000000000) {
                return res.status(400).json({ message: 'Số tiền thanh toán không hợp lệ' });
            }

            // Alphanumeric txn ref, unique per day
            const txnRef = `${String(cart._id)}${Date.now()}`.replace(/[^a-zA-Z0-9]/g, '').slice(0, 100);

            let vnpayResponse;
            try {
                // Official VNPay signing (fixes "Sai chữ ký" from npm vnpay URLSearchParams hash)
                vnpayResponse = buildPaymentUrl({
                    amount,
                    orderInfo: String(cart._id),
                    txnRef,
                ...
                console.error('paymentVnpay build error:', buildErr.message);
                return res.status(buildErr.statusCode || 500).json({ message: buildErr.message });
            }
    ...
    checkPaymentVnpay = async (req, res) => {
        try {
            ...
            let verified;
            try {
                verified = verifyReturnUrl({ ...req.query });
            } catch (verifyErr) {
            ...
            if (!verified?.isVerified || !verified?.isSuccess) {
                console.error('VNPay return rejected:', verified?.message, req.query?.vnp_ResponseCode);
                return res.redirect(`${redirectBase}/payments`);
            }
