...
function DoctorInbox() {
    ...
    const handleSubmitAnswer = async (id) => {
        ...
        try {
            setSubmittingId(id);
            const res = await request.put(`/api/doctor-inbox/inbox/${id}/answer`, { answer });
    ...
    return (
        <div className={cx('doctorPage')}>
            ...
            <p className={cx('pageDesc')}>
                Xem và trả lời câu hỏi từ khách hàng, nhân viên trên hệ thống.
                            ...
                            item.needsReply ? (
                                <div className={cx('qaAnswer')}>
                                    <strong>
                                        A (
                                        {item.isEscalated || item.escalatedToDoctor
                                            ? item.assignedDoctorName || item.answeredByName || 'Hỗ trợ'
                                            : item.answeredByName || 'Bác sĩ'}
                                        ):
                                    </strong>
