...
function ReminderPage() {
    ...
    const handleCreate = async () => {
        ...
        if (methods.length === 0) {
            alert('Vui lòng chọn phương thức thông báo');
            return;
        }
            ...
            <div className={cx('layout')}>
                                ...
                                <div className={cx('topCard')} key={item._id}>
                                    ...
                                    <div className={cx('cardActions')}>
                                        {isCompletedToday(item) ? (
                                            <button type="button" className={cx('completedBtn')} disabled>
                                                ✓ Hoàn thành
                                            </button>
                                        ) : (
                                            <button type="button" onClick={() => handleComplete(item._id)}>
                                                Hoàn thành
                                            </button>
                                        )}
                            ...
                            reminders.slice(0, 5).map((item, index) => (
                                <div className={cx('logItem')} key={item._id}>
                                    ...
                                    <div className={cx('logInfo')}>
                                        <h4>{item.title}</h4>
                                        <p>
                                            {index === 0 ? 'Hôm nay' : 'Gần đây'}, {item.times?.[0]}
                                        </p>
                                    </div>

                                    <span className={cx('logStatus', { missed: index === 1 })}>
                                        {index === 1 ? 'Bỏ lỡ' : 'Hoàn thành'}
                                    </span>
                                </div>
                            ))
                        ) : (
                ...
                <aside className={cx('right')}>
                    ...
                    <input
                        ...
                        placeholder="Ví dụ: Đến giờ thư giãn cơ"
                    />
                    ...
                    <input
                        ...
                        placeholder="Ví dụ: Đấm bóp vai gáy"
                    />
                    ...
                    <div className={cx('frequency')}>
                        <button
                            type="button"
                            className={cx({ active: frequency === 'daily' })}
                        ...
                        <button
                            type="button"
                            className={cx({ active: frequency === 'weekly' })}
                        ...
                        <button
                            type="button"
                            className={cx({ active: frequency === 'custom' })}
                ...
                <div className={cx('modalOverlay')} onClick={() => setShowReport(false)}>
                    <div className={cx('reportModal')} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className={cx('reportGrid')}>
                            ...
                            <div className={cx('reportCard')}>
                                <span>Nhắc qua Email</span>
                                <strong>{emailReminders}</strong>
                            </div>
