...
function ReviewPro({ dataPayments = [] }) {
    ...
    useEffect(() => {
        if (!selectedKey && reviewItems.length > 0) {
            setSelectedKey(reviewItems[0].key);
        }

        if (selectedKey && !reviewItems.some((item) => item.key === selectedKey)) {
            setSelectedKey(reviewItems[0]?.key || '');
        }
    ...
    useEffect(() => {
        const fetchMyReviews = async () => {
            try {
                ...
                const keys = (res.data || []).map((review) => {
                    return `${review.orderId}-${review.productIndex}`;
                });
    ...
    const handleSubmitReview = async () => {
        if (!order || !product) {
            toast.error('Không có dữ liệu đánh giá');
            return;
        }
        ...
        try {
            ...
            const res = await request.post('/api/reviews', formData, {
                headers: {
            ...
            <div className={cx('reviewGrid')}>
                <div className={cx('reviewSidebar')}>
                    <div className={cx('reviewCard')}>
                        <div className={cx('reviewCardHeader')}>
                        ...
                        <div className={cx('reviewProductList')}>
                            {reviewItems.map((item) => (
                                <button
                                    key={item.key}
                                    type="button"
                                    className={cx('reviewProductItem', {
                    ...
                    <div className={cx('reviewCard')}>
                        ...
                        <div className={cx('reviewInfo')}>
                            ...
                            <strong>#{order?._id?.slice(0, 12)}</strong>
                        </div>

                        <div className={cx('reviewInfo')}>
                            <span>Ngày mua</span>
                            <strong>
                                {order?.createdAt ? new Date(order.createdAt).toLocaleString('vi-VN') : 'Không có'}
                            </strong>
                        </div>

                        <div className={cx('reviewInfo')}>
                            <span>Trạng thái</span>
                            <div className={cx('successBadge', statusInfo.className)}>{statusInfo.text}</div>
                        </div>

                        <div className={cx('reviewInfo')}>
                            <span>Tổng tiền</span>
                            <strong>{formatMoney(order?.sumprice)}</strong>
                        </div>
                    </div>

                    <div className={cx('reviewCard')}>
                        <div className={cx('reviewCardHeader')}>
                            <h3>Sản phẩm đang đánh giá</h3>
                        </div>
