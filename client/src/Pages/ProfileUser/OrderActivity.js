...
function OrderActivity({ dataPayments = [], onViewDetail, onCancelSuccess }) {
    ...
    const processedPayments = useMemo(() => {
        ...
        source.forEach((item) => {
            ...
            const oldTime = oldItem?.updatedAt
                ? new Date(oldItem.updatedAt).getTime()
                : oldItem?.createdAt
                  ? new Date(oldItem.createdAt).getTime()
                  : 0;
        ...
        for (let i = 1; i <= totalPages; i++) {
            pages.push(
                <button
                    key={i}
                    type="button"
                    className={cx('pageBtn', { activePage: currentPage === i })}
                                        ...
                                        >
                                            ...
                                            <td>
                                                <div className={cx('productCell')}>
                                                    <span className={cx('productName')}>
                                                        {item?.products?.[0]?.nameProduct || 'Không có sản phẩm'}
                                                    </span>
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                <div className={cx('actionWrap')}>
                                                    <button
                                                        type="button"
                                                        className={cx('detailBtn', 'detailBtnFancy')}
